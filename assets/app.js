(() => {
  ...
  const currentView = () => 'library';
  const currentType = () => ['deep-dives','articles'].includes(params().get('type')) ? params().get('type') : 'deep-dives';
  let view = currentView(), type = currentType(), sector = params().get('sector') || 'All sectors', savedOnly = params().get('saved') === '1';
  ...
  function syncUrl(push = false) {
    ...
    if (type !== 'deep-dives' && type !== 'all') query.set('type',type);
    ...
    const url = (view === 'home' ? '/' : '/research/') + (query.size ? '?' + query : '');
  ...
  function render() {
    ...
    for (const card of cards) {
      const matchesType = type === 'articles' ? card.dataset.kind === 'General articles' : type === 'deep-dives' ? card.dataset.kind !== 'General articles' : true;
      const saved = window.LedgerReader?.isSaved(card.dataset.slug) || false;
      card.hidden = !((savedOnly || matchesType) && (sector === 'All sectors' || card.dataset.sector === sector) && terms.every(t => card.dataset.search.toLowerCase().includes(t)) && (!savedOnly || saved));
    ...
    document.querySelector('#result-count').textContent = visible + ' ' + (type === 'articles' || savedOnly ? (visible === 1 ? 'article' : 'articles') : (visible === 1 ? 'deep dive' : 'deep dives'));
    ...
    document.querySelector('.chips').hidden = type === 'articles' && !savedOnly;
    ...
    document.querySelector('#view-title').textContent = savedOnly ? 'Your reading list.' : 'Research';
  ...
  savedButton.addEventListener('click',() => { savedOnly = !savedOnly; sector = 'All sectors'; type=currentType(); render(); syncUrl(); });
  document.querySelectorAll('[data-type]').forEach(button=>button.addEventListener('click',()=>{type=button.dataset.type;sector='All sectors';savedOnly=false;render();syncUrl(true);}));
  document.querySelectorAll('[data-view="library"]').forEach(a => a.addEventListener('click',event => {
    if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
    event.preventDefault(); view = a.dataset.view; type='deep-dives'; sector = 'All sectors'; savedOnly = false; search.value = ''; syncUrl(true); render();
  }));
  window.addEventListener('popstate',() => { view=currentView();savedOnly=params().get('saved') === '1';type=currentType();sector=params().get('sector') || 'All sectors'; search.value=params().get('q') || ''; sort.value=params().get('sort') || 'new'; render(); });
  document.querySelector('#reset').addEventListener('click',() => { sector='All sectors';type='deep-dives';search.value='';savedOnly=false;render();syncUrl();search.focus(); });
